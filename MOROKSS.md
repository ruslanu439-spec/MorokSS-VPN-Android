...
Перед первым соединением клиент передаёт 96 КБ в обе стороны. Так он замечает
сеть, где TLS-handshake проходит, а поток потом зависает. Рабочая связка
endpoint, SNI, TLS-профиля и транспорта запоминается отдельно для мобильной
сети и Wi-Fi.
